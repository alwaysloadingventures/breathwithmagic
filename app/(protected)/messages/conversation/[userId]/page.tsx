...
import { ConversationThreadClient } from "./conversation-thread-client";
...
export async function generateMetadata({
  ...
  const partnerName =
    partner?.creatorProfile?.displayName || partner?.name || "User";

  return {
    title: `Chat with ${partnerName} | breathwithmagic`,
    description: `Direct message conversation with ${partnerName}`,
  };
}

/**
 * ConversationPage - Server component for a 1:1 conversation thread
 *
 * PRD: Thread view shows message history with user
        ...
        >
          <div className="mx-auto flex w-full max-w-3xl flex-1 flex-col overflow-hidden px-4 sm:px-6 lg:px-8">
            <ConversationThreadClient
              ...
              dmDisabledReason={dmDisabledReason}
            />
          </div>
        </main>
      </div>
